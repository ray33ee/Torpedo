...
mod torpedo_tests {
    ...
    fn test_cells_coms() {
        ...
        let index = generator.gen_range(0..sample.len());

        let ip = sample.get(index).unwrap();
        ...
        let stream = TcpStream::connect(format!("{}:{}", ip.0, ip.1)).unwrap();
        let mut stream = connector.connect("", stream).unwrap();

        TorCell::new(0, Command::Versions {version_list: VersionsVector::from(vec![3, 4]) }).into_stream(& mut stream, 3).unwrap();

        let versions = TorCell::from_stream(& mut stream, 3).unwrap();

        println!("{:?}", versions);

        let certs = TorCell::from_stream(& mut stream, 4).unwrap();

        println!("{:?}", certs);

        let auth = TorCell::from_stream(& mut stream, 4).unwrap();

        println!("{:?}", auth);

        let netinfo = TorCell::from_stream(& mut stream, 4).unwrap();

        println!("{:?}", netinfo);

        TorCell::new(0, Command::NetInfo {
            timestamp: Local::now(),
            other_ip: IpAddr::V4(Ipv4Addr::from_str(ip.0).unwrap()),
            this_ips: NLengthVector::from(vec![IpAddr::V4(Ipv4Addr::from_str("0.0.0.0").unwrap())]) }).into_stream(& mut stream, 4).unwrap();

        let half_handshake = [3u8; 20];

        TorCell::new(0x80000001, Command::CreateFast {onion_skin: half_handshake}).into_stream(& mut stream, 4).unwrap();

        let created_fast = TorCell::from_stream(& mut stream, 4).unwrap();
        ...
        if let Command::CreatedFast{ handshake_data } = created_fast.get_command() {
            ...
            cell_crypto.set_forward_digest(& mut relay_begin_dir).unwrap();

            let encrypted_begin = cell_crypto.encrypt(relay_begin_dir).unwrap();

            let relay_begin = TorCell::new(0x80000001, Command::Relay { contents: encrypted_begin });

            relay_begin.into_stream(& mut stream, 4).unwrap();

            //Read cell

            let begun = TorCell::from_stream(& mut stream, 4).unwrap();

            println!("cell data: {:?}", begun);

            if let crate::cells::Command::Relay { contents } = begun.get_command() {
                let mut contents = cell_crypto.decrypt(contents).unwrap();
                ...
                println!("Relay: {:?}", contents.get_payload());
            }
    ...
    fn test_cell_crypto() {
        ...
        cell.bin_serialise_into(data.as_mut()).unwrap();

        println!("relay testing: {:?}", data);

        crypto_cell.set_forward_digest(& mut cell).unwrap();
        ...
        let encrypted = crypto_cell.encrypt(cell).unwrap();
    ...
    fn second_cellcrypto() {
        ...
        let mut cell = crypto_cell.decrypt(&encrypted).unwrap();
    ...
    #[test]
    fn test_read_unpacked() {

        let mut cell = UnpackedCell::new(40, Some(vec![23, 6, 54, 23, 54, 6, 8, 43, 2, 45, 56, 42, 28, 5, 2, 13, 85]));

        let bytes = vec![40, 23, 6, 54, 23, 54, 6, 8, 43, 2, 45, 56, 42, 28, 5, 2, 13, 85];

        let mut ser = Vec::new();

        cell.read_to_end(& mut ser).unwrap();
    ...
    fn test_write_unpacked() {
        ...
        let mut cell = UnpackedCell::default();

        let bytes = [17, 87, 23, 72, 33, 38, 145, 234];

        cell.write_all(&bytes).unwrap();

        let bytes = [34, 62, 76, 129];

        cell.write_all(&bytes).unwrap();

        assert_eq!(cell.command(), 17);

        assert_eq!(cell.data(), vec![87, 23, 72, 33, 38, 145, 234, 34, 62, 76, 129]);
    }
